...
async def kviks(client, message):
	global number
	number = number + 1
	vopross = ['гей?', 'безмамный?', 'майнкрафтер?', 'влад а4?', 'ебал собак?', 'крутой?', 'клоун?', 'негр?', 'ебал стики?', 'сочный арбуз?', 'АААААА В ЧАТ УПАЛА БОМБА АДОЛМЛПРВЖОАПВП', 'Скорби гей?']
	kviks = [f'Квикс согласен 😁',f'Квикс не согласен 😲',f'Квикс не будет отвечать на это ☹',f'Квикс разозлился и отвечать на это не будет 😡',f'Квикс засмеялся 🤣',f'Квикс с хорошим настроением сказал что он согласен 😀 (РЕДКИЙ ОТВЕТ)',f'Квикс послал тебя нахуй!!🤬🤬 (РЕДКИЙ ОТВЕТ)',f'<b>Скорби гей (ЛЕГЕНДАРНЫЙ ОТВЕТ)</b>',f'<b>Квикс из за этого вопроса стал дед инсайдом 😔 (ХРОМАТИЧЕСКИЙ ОТВЕТ)</b>',f'Квикс затроллил Скорби 😎😎']
...
def mems(app, msg):
	...
	(Примичание: Они могут подгружатся дольше чем надо, смотря какая скорость у интернета.)
	 ...
	 2) Команда: ".татышоахуэл"
	 3) Команда: ".блядьнахуй"
...
def betalove(_, msg):
	...
	for i in range(1):
		...
		sleep(time)
		msg.edit(f"<b>я люблю когда волосатые мужики обмазываются маслом 🧈</b>")  # oran
		sleep(time)
		msg.edit(f"<b>я люблю когда волосатые мужики обмазываются маслом 🧈 <--- это масло</b>")  # 
		sleep(time)
		msg.edit(f"<b>я люблю когда волосатые мужики обмазываются маслом 🧈 <--- это масло (НЕ МЕД!1!!!11)</b>")  # orange
		sleep(time)
		msg.edit(f"<b>я люблю когда волосатые мужики обмазываются маслом 🧈 <--- это масло (а если это мед то только в наших мечтах)</b>")  # orange
		sleep(5)
		global number
		number = number + 1
